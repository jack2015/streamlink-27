...
class TwitchAPI:
    ...
    def call(self, data, schema=None, **kwargs):
        ...
        res = self.session.http.post(
            ...
            json=data,
            headers=mergedict(self.headers, kwargs.pop("headers", {})),
            **kwargs
        )
    ...
    def access_token(self, is_live, channel_or_vod):
        ...
        return self.call(query, acceptable_status=(200, 400, 401, 403), schema=validate.Schema(
            validate.any(
                validate.all(
                    {"error": validate.text, "message": validate.text},
...
class Twitch(Plugin):
    ...
    def _access_token(self, is_live, channel_or_vod):
        try:
            response = self.api.access_token(is_live, channel_or_vod)
            if response[0] != "token":
                error, message = response[1:]
                log.error("{0}: {1}".format(error or 'Error', message or 'Unknown error'))
                raise PluginError
            sig, token = response[1:]
        except (PluginError, TypeError):
