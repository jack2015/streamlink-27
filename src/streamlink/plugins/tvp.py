...
class TVP(Plugin):
    ...
    def _get_video_id(self, channel_id):
        # type: (Optional[str])
        items = self.session.http.get(
    ...
    def _get_vod(self, vod_id):
        log.info(self._URL_VOD.format(vod_id=vod_id))
    ...
    def _get_streams(self):
        if self.match.group("vod_id"):
            return self._get_vod(self.match.group("vod_id"))
        else:
            return self._get_live(self.match.group("channel_id"))
