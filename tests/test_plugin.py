...
class TestPluginMatcher(unittest.TestCase):
    ...
    def test_decorator(self):
        with self.assertRaises(TypeError) as cm:
            ...
            class Foo:
                pass
        m = re.search(r"(Foo )(?:.*?)(is not a Plugin)", str(cm.exception))
        if m:
            exception = "".join(m.groups())
        else:
            exception = str(cm.exception)
        self.assertEqual(exception, "Foo is not a Plugin")
...
class TestCookies:
    ...
    def test_load(self, session, plugin, plugincache, logger):
        # type: (Streamlink, Plugin, Mock, Mock)
        ...
        if is_py2:
            assert logger.debug.call_args_list == [call("Restored cookies: test-name2, test-name1")]
        else:
    ...
    def test_save(self, session, plugin, plugincache, logger):
        # type: (Streamlink, Plugin, Mock, Mock)
    ...
    @pytest.mark.parametrize("plugincache", [{}], indirect=True)
    def test_save_expires(self, session, plugin, plugincache):
        # type: (Streamlink, Plugin, Mock)
    ...
    def test_clear(self, session, plugin, plugincache):
        # type: (Streamlink, Plugin, Mock)
    ...
    def test_clear_filter(self, session, plugin, plugincache):
        # type: (Streamlink, Plugin, Mock)
        assert tuple(session.http.cookies.keys()) == ("test-name1", "test-name2")
    ...
    def test_unbound(self, method, err):
        # type: (str, str)
        plugin = Plugin("http://test.se")
        with pytest.raises(RuntimeError) as cm:
            getattr(plugin, method)()
        assert str(cm.value), "Cannot {0} cached cookies in unbound plugin".format(err)
